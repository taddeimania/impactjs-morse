...
ig.baked=true;ig.module('game.controllers.listener').requires('impact.game','impact.font').defines(function(){MORSE_DICT={a:'.-',b:'-...',c:'-.-.',d:'-..',e:'.',f:'..-.',g:'--.',h:'....',i:'..',j:'.---',k:'-.-',l:'.-..',m:'--',n:'-.',o:'---',p:'.--.',q:'--.-',r:'.-.',s:'...',t:'-',u:'..-',v:'...-',w:'.--',x:'-..-',y:'-.--',z:'--..',1:'.----',2:'..---',3:'...--',4:'....-',5:'.....',6:'-....',7:'--...',8:'---..',9:'----.',0:'-----',}
Listener=ig.Class.extend({cord:'',string:"",font:new ig.Font('media/font.png'),init:function(dit,dah,term,phrase){ig.input.bind(dit,'.');ig.input.bind(dah,'-');ig.input.bind(ig.KEY.SPACE,'space');ig.input.bind(ig.KEY.BACKSPACE,'backspace');ig.input.bind(term,'term');ig.input.bind(ig.KEY.ESC,'kill');this.phrase=phrase;this.activeTimer=new ig.Timer();},update:function(){if(this.activeTimer.delta()>.75||ig.input.pressed('term')){this.sendCord();}
if(ig.input.pressed('.')){this.updateCord('.');}else if(ig.input.pressed('-')){this.updateCord('-');}else if(ig.input.pressed('space')){this.addSpace();}else if(ig.input.pressed('backspace')){this.backSpace();}else if(ig.input.pressed('kill')){this.flushCord(true);}},draw:function(){var x=ig.system.width/2,y=ig.system.height/2;this.font.draw(this.string,x,40,ig.Font.ALIGN.CENTER);this.font.draw(this.cord,x,100,ig.Font.ALIGN.CENTER);},sendCord:function(){this.flushCord();this.activeTimer.reset();},updateCord:function(instruction){this.activeTimer.reset();this.validateCord();this.cord=this.cord+instruction;},validateCord:function(){if(this.cord.length>=6){this.flushCord();}},evaluateCord:function(){for(var key in MORSE_DICT){var value=MORSE_DICT[key];if(value===this.cord){this.string=this.string+key;return;}}},flushCord:function(escape){if(this.cord&&!escape){this.evaluateCord();}
this.cord=[];},addSpace:function(){this.string+=" ";},backSpace:function(){this.string=this.string.slice(0,this.string.length-1);},success:function(){if(this.phrase===this.string){return true;}}});});
...
ig.baked=true;ig.module('game.backgrounds.grass').requires('impact.game','game.backgrounds.mapTiles','game.backgrounds.baseBackground').defines(function(){GrassBackground=BaseBackground.extend({MAP:["9990000999","9298888929","9220000229","9200000029","9200000029","9200000029"],COLLISION:["1111111111","1011111101","1000000001","1000000001","1000000001","1000000001"],SHADOW:["0001002000","0300000030","0100000020","0100000020","0100000020","0100000020"],init:function(){this.parent();this.miscTiles=[[this.map.treeTall,[2,1]],[this.map.treeTall,[4,1]],[this.map.treeTall,[2,2]]];},});});

// lib/game/entities/tommy.js
ig.baked=true;ig.module('game.entities.tommy').requires('impact.game').defines(function(){EntityTommy=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/boy.png',101,171),size:{x:101,y:171},init:function(x,y,settings){this.tile=settings.tile;this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.currentAnim=this.anims.idle;},update:function(){this.parent();if(ig.input.pressed('up')){this.move('up');}else if(ig.input.pressed('down')){this.move('down');}else if(ig.input.pressed('right')){this.move('right');}else if(ig.input.pressed('left')){this.move('left');}},move:function(direction){if(direction==='up'){if(!ig.game.background.checkAboveTile(this.tile[0],this.tile[1])){this.tile[0]-=1;}}else if(direction==='down'){if(!ig.game.background.checkBelowTile(this.tile[0],this.tile[1])){this.tile[0]+=1;}}else if(direction==='right'){if(!ig.game.background.checkRightTile(this.tile[0],this.tile[1])){this.tile[1]+=1;}}else if(direction==='left'){if(!ig.game.background.checkLeftTile(this.tile[0],this.tile[1])){this.tile[1]-=1;}}
var newCoords=ig.game.background.getCharCoords(this.tile[0],this.tile[1]);this.pos.x=newCoords.x;this.pos.y=newCoords.y;}});});
...
ig.baked=true;ig.module('game.entities.enter').requires('game.entities.baseTouch').defines(function(){EntityEnter=EntityBaseTouch.extend({animSheet:new ig.AnimationSheet('media/touchEtcEntity.png',320,214),size:{x:320,y:214},update:function(){this.parent();if(this.clickOnMe()){ig.game.listener.sendCord();}},});});

// lib/game/entities/space.js
ig.baked=true;ig.module('game.entities.space').requires('game.entities.baseTouch').defines(function(){EntitySpace=EntityBaseTouch.extend({animSheet:new ig.AnimationSheet('media/touchEtcEntity.png',320,214),size:{x:320,y:214},update:function(){this.parent();if(this.clickOnMe()){ig.game.listener.addSpace();}},});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.controllers.listener','game.backgrounds.grass','game.entities.tommy','game.entities.dah','game.entities.dit','game.entities.backspace','game.entities.enter','game.entities.space').defines(function(){VERSION="0.0.2.3-13.1"
MainGame=ig.Game.extend({versionFont:new ig.Font('media/font.png'),draw:function(){this.parent();this.versionFont.draw('Build: '+VERSION,0,0,ig.Font.ALIGN.LEFT);}});MyGame=MainGame.extend({instructions:new ig.Font('media/font.png'),init:function(){this.background=new GrassBackground();this.listener=new Listener(ig.KEY.Z,ig.KEY.M,ig.KEY.ENTER,'sos');ig.input.bind(ig.KEY.Z,'.');ig.input.bind(ig.KEY.M,'-');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.LEFT_ARROW,'left');this.dit=ig.game.spawnEntity('EntityDit',0,0);this.dah=ig.game.spawnEntity('EntityDah',640,0);this.space=ig.game.spawnEntity('EntitySpace',320,0);this.backspace=ig.game.spawnEntity('EntityBackspace',320,428);this.enter=ig.game.spawnEntity('EntityEnter',320,214);var tommySpawn=this.background.getCharCoords(4,2);this.tommy=ig.game.spawnEntity('EntityTommy',tommySpawn.x,tommySpawn.y,{tile:[4,2]});ig.input.bind(ig.KEY.MOUSE1,'click');},update:function(){this.parent();this.listener.update();if(this.listener.success()){console.log('SUCCESS!');}},draw:function(){this.parent();this.background.draw();this.listener.draw();this.tommy.draw();this.background.drawExtra();}});ig.main('#canvas',MyGame,60,960,640,1);});