...
// lib/game/controllers/listener.js
ig.baked=true;ig.module('game.controllers.listener').requires('impact.game','impact.font').defines(function(){MORSE_DICT={a:'.-',b:'-...',c:'-.-.',d:'-..',e:'.',f:'..-.',g:'--.',h:'....',i:'..',j:'.---',k:'-.-',l:'.-..',m:'--',n:'-.',o:'---',p:'.--.',q:'--.-',r:'.-.',s:'...',t:'-',u:'..-',v:'...-',w:'.--',x:'-..-',y:'-.--',z:'--..',1:'.----',2:'..---',3:'...--',4:'....-',5:'.....',6:'-....',7:'--...',8:'---..',9:'----.',0:'-----'};Listener=ig.Class.extend({cord:'',string:"",font:new ig.Font('media/font.png'),init:function(dit,dah,term,phrase){ig.input.bind(dit,'.');ig.input.bind(dah,'-');ig.input.bind(ig.KEY.BACKSPACE,'backspace');ig.input.bind(term,'term');ig.input.bind(ig.KEY.ESC,'kill');this.phrase=phrase;this.activeTimer=new ig.Timer();},update:function(){if(ig.input.pressed('.')){this.updateCord('.');}else if(ig.input.pressed('-')){this.updateCord('-');}else if(ig.input.pressed('backspace')){this.backSpace();}else if(ig.input.pressed('kill')){this.flushCord(true);}
if(this.activeTimer.delta()>.75||ig.input.pressed('term')){this.sendCord();}},draw:function(){var x=ig.system.width/2,y=ig.system.height/2;this.font.draw(this.string,x,40,ig.Font.ALIGN.CENTER);this.font.draw(this.cord,x,100,ig.Font.ALIGN.CENTER);},sendCord:function(){this.flushCord();this.activeTimer.reset();},updateCord:function(instruction){this.activeTimer.reset();this.validateCord();this.cord=this.cord+instruction;},validateCord:function(){if(this.cord.length>=5){this.flushCord(true);}},evaluateCord:function(){for(var key in MORSE_DICT){var value=MORSE_DICT[key];if(value===this.cord){this.string=this.string+key;return;}}},flushCord:function(escape){if(this.cord&&!escape){this.evaluateCord();}
this.cord=[];},backSpace:function(){this.string=this.string.slice(0,this.string.length-1);},success:function(){if(this.phrase===this.string){return true;}}});});

// lib/game/backgrounds/mapTiles.js
ig.baked=true;ig.module('game.backgrounds.mapTiles').requires('impact.game').defines(function(){MapTiles=ig.Class.extend({loaded:false,init:function(){this.treeTall=new ig.Image('media/treeTall.png');this.blank=new ig.Image('media/blank.png');this.grassBlock=new ig.Image('media/grassBlock.png');this.dirtBlock=new ig.Image('media/dirtBlock.png');this.woodBlock=new ig.Image('media/woodBlock.png');this.plainBlock=new ig.Image('media/plainBlock.png');this.stoneBlock=new ig.Image('media/stoneBlock.png');this.waterBlock=new ig.Image('media/waterBlock.png');this.wallBlock=new ig.Image('media/wallBlock.png');this.stoneTallBlock=new ig.Image('media/stoneTallBlock.png');this.wallTallBlock=new ig.Image('media/wallTallBlock.png');this.shadowEast=new ig.Image('media/shadowEast.png');this.shadowWest=new ig.Image('media/shadowWest.png');this.tileMap=[this.grassBlock,this.dirtBlock,this.woodBlock,this.plainBlock,this.stoneBlock,this.waterBlock,this.wallBlock,this.blank,this.wallTallBlock,this.stoneTallBlock]},tilesLoaded:function(){if(!this.loaded){for(var i=0;i<this.tileMap.length;i++){if(!this.tileMap[i].loaded){return false;}}}
this.loaded=true;return true;}});});

// lib/game/backgrounds/baseBackground.js
ig.baked=true;ig.module('game.backgrounds.baseBackground').requires('impact.game','game.backgrounds.mapTiles').defines(function(){SHADOW_DEF={1:function(self){return[self.map.shadowWest]},2:function(self){return[self.map.shadowEast]},3:function(self){return[self.map.shadowEast,self.map.shadowWest]}};BaseBackground=ig.Class.extend({init:function(){this.map=new MapTiles();},draw:function(){if(this.map.tilesLoaded()){this.drawMap();}},drawMap:function(){for(var j=0;j<this.MAP.length;j++){for(var i=0;i<this.MAP[j].length;i++){var tile=parseInt(this.MAP[j][i]);var currentTile=this.getCurrentCoords(j,i)
this.map.tileMap[this.MAP[j][i]].draw(currentTile.x,currentTile.y);var shadowTile=this.SHADOW[j][i];if(shadowTile>0){var shadowList=SHADOW_DEF[shadowTile](this);for(var k=0;k<shadowList.length;k++){shadowList[k].draw(currentTile.x,currentTile.y);}}}}},drawExtra:function(){for(var i=0;i<this.miscTiles.length;i++){var tileObj=this.miscTiles[i][0];var coords=this.miscTiles[i][1];var tileCoords=this.getCurrentCoords(coords[0],coords[1]);tileObj.draw(tileCoords.x,tileCoords.y-65);}},getCharCoords:function(j,i){var currentTile=this.getCurrentCoords(j,i);currentTile.y-=50;return currentTile;},getCurrentCoords:function(j,i){return{x:(i*100)-25,y:(j*82)+27};},checkBelowTile:function(j,i){if(j>4){return true;}
return parseInt(this.COLLISION[j+1][i])>0;},checkAboveTile:function(j,i){if(j===0){return true;}
return parseInt(this.COLLISION[j-1][i])>0;},checkRightTileForShadow:function(j,i){if(this.checkRightTile(j,i)){return parseInt(this.MAP[j][i+1])>7;}
return parseInt(this.COLLISION[j][i+1])>0;},checkLeftTileForShadow:function(j,i){if(this.checkLeftTile(j,i)){return parseInt(this.MAP[j][i-1])>7;}
return parseInt(this.COLLISION[j][i-1])>0;},checkRightTile:function(j,i){if(i>9){return true;}
return parseInt(this.COLLISION[j][i+1])>0;},checkLeftTile:function(j,i){if(i===0){return true;}
return parseInt(this.COLLISION[j][i-1])>0;},tilesLoaded:function(){if(!this.map.loaded){for(var i;i<this.tileMap.length;i++){if(!this.map.tileMap.loaded){return false;}}}
this.map.loaded=true;return true;}});});

// lib/game/backgrounds/grass.js
ig.baked=true;ig.module('game.backgrounds.grass').requires('impact.game','game.backgrounds.mapTiles','game.backgrounds.baseBackground').defines(function(){GrassBackground=BaseBackground.extend({MAP:["7777777777","7777777777","7777777777","0000000004","0000000004","0000000004"],COLLISION:["1111111111","1111111111","1111111111","1000000001","1000000001","1000000001"],SHADOW:["0000000000","0000000000","0000000000","0000000000","0000000000","0000000000"],init:function(){this.parent();this.miscTiles=[];},});});

// lib/game/entities/tommy.js
ig.baked=true;ig.module('game.entities.tommy').requires('impact.game').defines(function(){EntityTommy=ig.Entity.extend({type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,animSheet:new ig.AnimationSheet('media/boy.png',101,171),size:{x:101,y:171},init:function(x,y,settings){this.collided=false;this.jumpOffset=1.2;this.checking=false;this.tile=settings.tile;this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.currentAnim=this.anims.idle;},update:function(){this.parent();if(ig.input.pressed('check')&&!this.checking){ig.game.listener.sendCord();this.validate();}},check:function(){this.parent();if(this.jumping){this.jumping=false;this.collided=true;}else{this.collided=false;}},validate:function(){this.checking=true;ig.game.bug=ig.game.spawnEntity('EntityBug',960,325);if(ig.game.listener.success()){this.originalY=this.pos.y;this.jumpingTimer=new ig.Timer();this.jumping=true;}}});});

// lib/game/entities/bug.js
ig.baked=true;ig.module('game.entities.bug').requires('impact.game').defines(function(){EntityBug=ig.Entity.extend({type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,animSheet:new ig.AnimationSheet('media/bug.png',101,171),size:{x:101,y:171},init:function(x,y,settings){this.tile=settings.tile;this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.currentAnim=this.anims.idle;},update:function(){this.parent();this.pos.x-=20;}});});

// lib/game/entities/baseTouch.js
ig.baked=true;ig.module('game.entities.baseTouch').requires('impact.game').defines(function(){EntityBaseTouch=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/touchEntity.png',320,640),size:{x:320,y:640},init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.currentAnim.alpha=0;},update:function(){this.parent();if(this.clickOnMe()){this.currentAnim.alpha=1;}},draw:function(){this.parent();if(this.currentAnim.alpha>.1){this.currentAnim.alpha-=.25;}else if(this.currentAnim.alpha<.1){this.currentAnim.alpha=0;}},clickOnMe:function(){return ig.input.pressed('click')&&(ig.input.mouse.y>this.pos.y&&ig.input.mouse.y<this.pos.y+this.size.y)&&(ig.input.mouse.x>this.pos.x&&ig.input.mouse.x<this.pos.x+this.size.x);},});});

// lib/game/entities/dah.js
ig.baked=true;ig.module('game.entities.dah').requires('game.entities.baseTouch').defines(function(){EntityDah=EntityBaseTouch.extend({update:function(){this.parent();if(this.clickOnMe()){ig.game.listener.updateCord('-');}},});});

// lib/game/entities/dit.js
ig.baked=true;ig.module('game.entities.dit').requires('game.entities.baseTouch').defines(function(){EntityDit=EntityBaseTouch.extend({update:function(){this.parent();if(this.clickOnMe()){ig.game.listener.updateCord('.');}},});});

// lib/game/entities/backspace.js
ig.baked=true;ig.module('game.entities.backspace').requires('game.entities.baseTouch').defines(function(){EntityBackspace=EntityBaseTouch.extend({animSheet:new ig.AnimationSheet('media/touchEtcEntity.png',320,214),size:{x:320,y:214},update:function(){this.parent();if(this.clickOnMe()){ig.game.listener.backSpace();}},});});

// lib/game/entities/enter.js
ig.baked=true;ig.module('game.entities.enter').requires('game.entities.baseTouch').defines(function(){EntityEnter=EntityBaseTouch.extend({animSheet:new ig.AnimationSheet('media/touchEtcEntity.png',320,214),size:{x:320,y:214},update:function(){this.parent();if(this.clickOnMe()){ig.game.listener.sendCord();ig.game.tommy.validate();}},});});

// lib/game/entities/space.js
ig.baked=true;ig.module('game.entities.space').requires('game.entities.baseTouch').defines(function(){EntitySpace=EntityBaseTouch.extend({animSheet:new ig.AnimationSheet('media/touchEtcEntity.png',320,214),size:{x:320,y:214},update:function(){this.parent();if(this.clickOnMe()){ig.game.listener.sendCord();}},});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.controllers.listener','game.backgrounds.grass','game.entities.tommy','game.entities.bug','game.entities.dah','game.entities.dit','game.entities.backspace','game.entities.enter','game.entities.space').defines(function(){VERSION="0.0.3.3-24.2"
MainGame=ig.Game.extend({versionFont:new ig.Font('media/font.png'),draw:function(){this.parent();this.versionFont.draw('Build: '+VERSION,0,0,ig.Font.ALIGN.LEFT);}});MyGame=MainGame.extend({instructions:new ig.Font('media/font.png'),init:function(){this.background=new GrassBackground();this.listener=new Listener(ig.KEY.Z,ig.KEY.M,ig.KEY.SPACE,'sos');ig.input.bind(ig.KEY.Z,'.');ig.input.bind(ig.KEY.M,'-');ig.input.bind(ig.KEY.ENTER,'check');this.dit=ig.game.spawnEntity('EntityDit',0,0);this.dah=ig.game.spawnEntity('EntityDah',640,0);this.space=ig.game.spawnEntity('EntitySpace',320,0);this.backspace=ig.game.spawnEntity('EntityBackspace',320,428);this.enter=ig.game.spawnEntity('EntityEnter',320,214);var tommySpawn=this.background.getCharCoords(4,3);this.tommy=ig.game.spawnEntity('EntityTommy',tommySpawn.x,tommySpawn.y,{tile:[4,3]});ig.input.bind(ig.KEY.MOUSE1,'click');},update:function(){this.parent();this.listener.update();if(this.tommy.collided||(this.bug&&this.bug.pos.x<-100)){this.killBug();}},draw:function(){this.parent();this.background.draw();this.listener.draw();this.tommy.draw();if(this.bug){this.bug.draw();}
this.instructions.draw('Instructions:',0,100,ig.Font.ALIGN.LEFT);this.instructions.draw('   Z : .',27,130,ig.Font.ALIGN.LEFT);this.instructions.draw('   M : -',18,160,ig.Font.ALIGN.LEFT);this.instructions.draw(' ENTER : REGISTER',9,190,ig.Font.ALIGN.LEFT);},killBug:function(){this.listener.string="";this.tommy.checking=false;this.tommy.collided=false;this.bug.kill()
this.bug=undefined;}});ig.main('#canvas',MyGame,60,960,640,1);});