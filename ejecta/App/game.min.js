...
ig.baked=true;ig.module('game.backgrounds.grass').requires('impact.game').defines(function(){MAP=["9997777999","9100000009","9100000009","9100000009","9100000009","9100000009"]
GrassBackground=ig.Class.extend({init:function(){this.boy=new ig.Image('media/boy.png');this.treeTall=new ig.Image('media/treeTall.png');this.blank=new ig.Image('media/blank.png');this.grassBlock=new ig.Image('media/grassBlock.png');this.dirtBlock=new ig.Image('media/dirtBlock.png');this.woodBlock=new ig.Image('media/woodBlock.png');this.plainBlock=new ig.Image('media/plainBlock.png');this.stoneBlock=new ig.Image('media/stoneBlock.png');this.waterBlock=new ig.Image('media/waterBlock.png');this.wallBlock=new ig.Image('media/wallBlock.png');this.stoneTallBlock=new ig.Image('media/stoneTallBlock.png');this.wallTallBlock=new ig.Image('media/wallTallBlock.png');this.shadowEast=new ig.Image('media/shadowEast.png');this.shadowWest=new ig.Image('media/shadowWest.png');this.tileMap=[this.grassBlock,this.dirtBlock,this.woodBlock,this.plainBlock,this.stoneBlock,this.waterBlock,this.wallBlock,this.blank,this.wallTallBlock,this.stoneTallBlock]},draw:function(){for(var j=0;j<MAP.length;j++){for(var i=0;i<MAP[j].length;i++){this.tileMap[MAP[j][i]].draw((i*100)-25,(j*82)+27);if(MAP[j][i]!=='9'){if(i>0){if(MAP[j][i-1]==='9'||MAP[j][i-1]==='8'){this.shadowWest.draw((i*100)-25,(j*82)+27);}}
if(i<9){if(MAP[j][i+1]==='9'||MAP[j][i+1]==='8'){this.shadowEast.draw((i*100)-25,(j*82)+27);}}}
if(j===5&&i===3){this.boy.draw((i*100)-25,(j*82)-40);}
if(j===5&&i===1){this.treeTall.draw((i*100)-25,(j*82)-27);}}}}});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.controllers.listener','game.backgrounds.grass','game.entities.dah','game.entities.dit','game.entities.backspace','game.entities.enter','game.entities.space').defines(function(){VERSION="0.0.2.3-7.3"
MainGame=ig.Game.extend({versionFont:new ig.Font('media/font.png'),draw:function(){this.parent();this.versionFont.draw('Build: '+VERSION,0,0,ig.Font.ALIGN.LEFT);}});MyGame=MainGame.extend({instructions:new ig.Font('media/font.png'),init:function(){this.background=new GrassBackground();this.listener=new Listener(ig.KEY.Z,ig.KEY.M,ig.KEY.ENTER,'sos');ig.input.bind(ig.KEY.Z,'.');ig.input.bind(ig.KEY.M,'-');this.dit=ig.game.spawnEntity('EntityDit',0,0);this.dah=ig.game.spawnEntity('EntityDah',640,0);this.space=ig.game.spawnEntity('EntitySpace',320,0);this.backspace=ig.game.spawnEntity('EntityBackspace',320,428);this.enter=ig.game.spawnEntity('EntityEnter',320,214);ig.input.bind(ig.KEY.MOUSE1,'click');},update:function(){this.parent();this.listener.update();if(this.listener.success()){console.log('SUCCESS!');}},draw:function(){this.parent();this.background.draw();this.listener.draw();}});ig.main('#canvas',MyGame,60,960,640,1);});